...
Modified version to use Anthropic's Claude API instead of OpenAI
"""
...
# Monkey patch to use Anthropic utils
...
# STEP 2: Import the Anthropic adapter
from pageindex import utils_anthropic as utils_replacement
...
# STEP 5: Now import the functions - they will automatically use the Anthropic adapter
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Process PDF or Markdown document with Anthropic Claude'
    )
    ...
    print("=" * 70)
    print("PageIndex with Anthropic Claude")
    print("=" * 70)
    print(f"Model: {args.model}")
    print(f"API Key: {os.getenv('ANTHROPIC_API_KEY')[:20]}..." if os.getenv('ANTHROPIC_API_KEY') else "Not set")
    print("=" * 70)
    ...
    if args.pdf_path:
        ...
        output_file = f"results/{os.path.basename(args.pdf_path).replace('.pdf', '_tree_claude.json')}"
        ...
        # Process Markdown with Claude (async)
        import asyncio
        ...
        # Save result
        os.makedirs('results', exist_ok=True)
        output_file = f"results/{os.path.basename(args.md_path).replace('.md', '_tree_claude.json')}"
