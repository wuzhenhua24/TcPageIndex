"""
Anthropic API adapter for PageIndex
This module adapts Anthropic's Claude API to work with PageIndex
"""
...
def get_anthropic_model_name(openai_model):
    """Convert OpenAI model name to Anthropic model name"""
    return ANTHROPIC_MODEL_MAP.get(openai_model, "claude-3-5-sonnet-20241022")

def count_tokens(text, model=None):
    """
    Count tokens in text
    Note: For Anthropic, we use tiktoken as approximation
    """
    if not text:
        return 0
    if HAS_TIKTOKEN:
...
def call_anthropic_api(model_name, prompt, api_key=None, temperature=0, max_tokens=4096, chat_history=None, api_base=None):
    ...
    if "content" in result and len(result["content"]) > 0:
        ...
        finish_reason = "finished" if stop_reason == "end_turn" else "max_output_reached"
...
def ChatGPT_API_with_finish_reason(model, prompt, api_key=None, chat_history=None):
    """
    Anthropic adapter for ChatGPT API with finish reason
    """
    max_retries = 10
    anthropic_model_name = get_anthropic_model_name(model)

    for i in range(max_retries):
        try:
            text, finish_reason = call_anthropic_api(
...
def ChatGPT_API(model, prompt, api_key=None, chat_history=None):
    """
    Anthropic adapter for ChatGPT API (main function)
    """
...
async def ChatGPT_API_async(model, prompt, api_key=None):
    """
    Async version for Anthropic API
    """
    max_retries = 10
    anthropic_model_name = get_anthropic_model_name(model)
    ...
    for i in range(max_retries):
        try:
            ...
            print(f'************* Retrying Async (Anthropic) ************* {e}')
            logging.error(f"Anthropic Async Error: {e}")
            if i < max_retries - 1:
